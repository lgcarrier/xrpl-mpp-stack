...
from xrpl_mpp_facilitator.gateway_auth import (
...
def create_app(
    ...
    app.add_middleware(BodySizeLimitMiddleware, max_body_bytes=active_settings.MAX_REQUEST_BODY_BYTES)
    ...
    def _payment_problem(detail: str, challenge_id: str | None = None) -> JSONResponse:
    ...
    async def supported() -> SupportedResponse:
        return SupportedResponse(methods=active_xrpl_service.supported_methods())

    @app.post("/charge", response_model=Receipt)
    @limiter.limit("30/minute", key_func=_payment_rate_limit_key)
    async def charge(request: Request, body: ChargeRequest) -> Receipt:
    ...
    async def session(request: Request, body: SessionRequest) -> Receipt:
        _require_authenticated_gateway(request)
        try:
