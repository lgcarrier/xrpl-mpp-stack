...
from xrpl.models.transactions import Payment, PaymentFlag, TrustSet, TrustSetFlag
...
from xrpl_mpp_core import RLUSD_CODE, RLUSD_HEX, normalize_currency_code
from xrpl_mpp_core.testnet_rpc import TESTNET_NETWORK_ID, probe_rpc_network_id
...
CACHED_WALLET_RESUME_HINT = "rerun the same command without --wallet-file"
LIQUIDITY_ENGINE_RESULTS = frozenset({"tecPATH_DRY", "tecPATH_PARTIAL"})
...
def request_testnet_xrp(address: str) -> None:
    request = Request(
        ...
        headers={"content-type": "application/json", "user-agent": "xrpl-mpp-stack/0.2.0"},
        method="POST",
    )
    try:
...
def _simulate_exact_payment(
    ...
    *,
    ...
    if result in LIQUIDITY_ENGINE_RESULTS:
        raise LiquidityUnavailableError(
            f"RLUSD conversion simulation found no complete route ({result})"
        )
    if result != "tesSUCCESS":
...
def fund_rlusd_wallet(
    ...
    *,
    ...
    if target_rlusd > DEFAULT_TRUSTLINE_LIMIT:
        raise SpendLimitExceededError(
            f"target RLUSD balance exceeds trust-line limit {DEFAULT_TRUSTLINE_LIMIT}"
        )
...
def _add_recovery_context(
    ...
    *,
    ...
    cached_wallet_selected: bool,
    address: str | None,
) -> None:
    if wallet_path is not None:
        payload["walletFile"] = str(wallet_path)
    elif cached_wallet_selected:
        payload["resumeHint"] = CACHED_WALLET_RESUME_HINT
    if address is not None:
        payload["address"] = address


def _print_recovery_context(
    *,
    wallet_path: Path | None,
    cached_wallet_selected: bool,
    address: str | None,
    ...
    if wallet_path is not None:
        ...
        print(f"Resume: {CACHED_WALLET_RESUME_HINT}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    selected_wallet_path: Path | None = None
    cached_wallet_selected = False
    ...
    try:
        ...
        if args.target_rlusd > DEFAULT_TRUSTLINE_LIMIT:
            raise SpendLimitExceededError(
                f"target RLUSD balance exceeds trust-line limit {DEFAULT_TRUSTLINE_LIMIT}"
            )
        ...
        with private_file_lock(lock_path):
            ...
            if args.new_wallet or args.wallet_file is not None:
                selected_wallet_path = wallet_path
            else:
                cached_wallet_selected = True
        ...
        _add_recovery_context(
            ...
            cached_wallet_selected=cached_wallet_selected,
        ...
        payload = {"status": "refused", "error": str(exc)}
        _add_recovery_context(
            payload,
            wallet_path=selected_wallet_path,
            cached_wallet_selected=cached_wallet_selected,
